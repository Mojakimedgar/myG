 ...
 */
export async function collectAndUpdateLocation(
  userId: string
...
export function startLocationTracking(
  userId: string,
  onUpdate?: (lat: number, lon: number, address: string) => void,
  ...
  const handlePosition = async (pos: GeolocationPosition) => {
    ...
    console.log(
      `[Location] ${latitude.toFixed(5)}, ${longitude.toFixed(5)} (±${accuracy.toFixed(0)}m)`
    );
