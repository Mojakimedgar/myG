 ...
 */
export function generateMockRoutePath(
  startLat: number,
  startLon: number,
  endLat: number,
  endLon: number,
 ...
 */
export function createMockRecordedRoute(
  kidId: string,
  ...
  const routePoints = generateMockRoutePath(
    startLat,
    startLon,
    endLat,
    endLon,
 ...
 */
export function generateDeviatedRoutePath(
  normalRoutePath: RoutePoint[],
  deviationPercentage: number = 50,
  maxDeviationMeters: number = 300
): RoutePoint[] {
  if (normalRoutePath.length === 0) return [];
...
): { matchPercentage: number; closestDistance: number } {
 ...
 */
export function logRouteInfo(route: RecordedRoute): void {
  ...
  console.log(
    "Duration:",
    Math.round(route.duration / 60000),
    "minutes"
  );
  console.log(
    "Avg Speed:",
    route.avgSpeed ? (route.avgSpeed * 3.6).toFixed(1) : "N/A",
    "km/h"
  );
  console.log(
    "Confidence:",
    route.confidence ? (route.confidence * 100).toFixed(1) : "N/A",
    "%"
  );
 ...
 */
export function logDeviationInfo(deviation: RouteDeviation): void {
  console.group(`⚠️ Deviation: ${deviation.severity}`);
  console.log("ID:", deviation.id);
  console.log(
    "Distance Off Route:",
    Math.round(deviation.deviationDistance),
    "m"
  );
  console.log(
    "Percentage:",
...
export function compareRoutes(
  route1: RecordedRoute,
  route2: RecordedRoute
): void {
  ...
  console.log(
    "Route 1:",
    route1.avgSpeed ? (route1.avgSpeed * 3.6).toFixed(1) : "N/A",
    "km/h"
  );
  console.log(
    "Route 2:",
    route2.avgSpeed ? (route2.avgSpeed * 3.6).toFixed(1) : "N/A",
    "km/h"
  );
  console.groupEnd();
 ...
 */
export function analyzeRoutes(routes: RecordedRoute[]): {
  count: number;
  avgDistance: number;
  avgDuration: number;
  minDistance: number;
 ...
 */
export function exportRouteAsJSON(route: RecordedRoute): string {
 ...
 */
export function generateRouteReport(route: RecordedRoute): string {
  const lines = [
    "═".repeat(50),
 ...
 */
export function enableRouteTrackingDebug(): void {
  console.log(
    "%c🚀 Route Tracking Debug Mode Enabled",
    "font-size: 16px; font-weight: bold; color: #4CAF50;"
  );
  console.log(
    "Available utilities: generateMockRoutePath, createMockRecordedRoute, logRouteInfo, etc."
  );

  // Make utilities globally available in dev
  if (typeof (window as any) !== "undefined") {
    (window as any).__routeTrackingDebug = {
...
export function runQuickTest(): void {
  ...
  try {
    ...
    const route1 = createMockRecordedRoute(
      "test_kid_1",
      "test_zone_1",
      "Test School",
      40.7128,
    ...
    const route2 = createMockRecordedRoute(
      "test_kid_1",
      "test_zone_1",
      "Test School",
      40.7128,
    ...
    const deviatedPath = generateDeviatedRoutePath(
      route1.routePoints,
      30,
      200
    );
