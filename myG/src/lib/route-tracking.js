...
export function calculateRouteTotalDistance(points) {
    ...
    for (let i = 0; i < points.length - 1; i++) {
        totalDistance += haversineDistance(points[i].latitude, points[i].longitude, points[i + 1].latitude, points[i + 1].longitude);
    }
 ...
 */
export function calculateRouteDuration(points) {
    if (points.length < 2)
        return 0;
    return (points[points.length - 1].timestamp.getTime() -
 ...
 */
export function findClosestPointOnRoute(currentLocation, routePoints) {
    let minDistance = Infinity;
    let closestIndex = 0;
    routePoints.forEach((point, index) => {
        const distance = haversineDistance(currentLocation.latitude, currentLocation.longitude, point.latitude, point.longitude);
 ...
 */
export function calculateRouteDeviation(currentLocation, recordedRoute) {
    const { distance, index } = findClosestPointOnRoute(currentLocation, recordedRoute.routePoints);
 ...
 */
export function getDeviationSeverity(deviationMeters) {
    if (deviationMeters < MINOR_DEVIATION_METERS)
        return "minor";
    if (deviationMeters < MAJOR_DEVIATION_METERS)
        return "moderate";
    return "major";
}
/**
 ...
 */
export function isMovingTowardDestination(currentLocation, previousLocation, destination) {
    const currentDist = haversineDistance(currentLocation.latitude, currentLocation.longitude, destination.latitude, destination.longitude);
    const previousDist = haversineDistance(previousLocation.latitude, previousLocation.longitude, destination.latitude, destination.longitude);
 ...
 */
export function estimateTimeToDestination(currentLocation, destination, averageSpeed = null) {
    const remaining = haversineDistance(currentLocation.latitude, currentLocation.longitude, destination.latitude, destination.longitude);
 ...
 */
export function calculateRouteMatchPercentage(currentPoints, recordedRoute) {
    if (currentPoints.length === 0)
        return 0;
    if (recordedRoute.routePoints.length < MIN_POINTS_FOR_ROUTE)
        return 0;
    ...
    currentPoints.forEach((currentPoint) => {
        const { distance } = findClosestPointOnRoute(currentPoint, recordedRoute.routePoints);
 ...
 */
export function createRecordedRoute(kidId, destinationId, destinationName, points, dayOfWeek) {
 ...
 */
export function calculateRouteConfidence(points) {
    if (points.length < MIN_POINTS_FOR_ROUTE)
        return 0;
    ...
    if (points.length > 1) {
        ...
        intervalConsistency = Math.max(0, 1 - stdDev / avgInterval);
    }
    // Check for accuracy (GPS precision)
    const avgAccuracy = points.reduce((sum, p) => sum + (p.accuracy || 10), 0) / points.length;
    ...
    if (speeds.length > 2) {
        ...
        const speedVariance = speeds.reduce((sum, speed) => sum + Math.pow(speed - avgSpeed, 2), 0) /
            speeds.length;
        const speedStdDev = Math.sqrt(speedVariance);
        speedConsistency = Math.max(0, 1 - speedStdDev / (avgSpeed + 0.1));
    }
    // Weighted average
    return (intervalConsistency * 0.3 +
        accuracyScore * 0.4 +
...
export function calculateRouteStatistics(routes) {
    ...
    const mostCommonDayOfWeek = Object.keys(dayFrequency).reduce((a, b) => dayFrequency[parseInt(b)] > dayFrequency[parseInt(a)] ? b : a);
    return {
        kidId,
        destinationId,
        ...
        avgDistance,
        avgDuration,
        mostCommonDayOfWeek: parseInt(mostCommonDayOfWeek),
 ...
 */
export function suggestRouteRecalculation(multipleDeviations, currentLocation, destination) {
    // If > 30% of recent points deviate, suggest reroute
    if (multipleDeviations.length === 0)
        return false;
    const majorDeviations = multipleDeviations.filter((d) => d.severity === "major").length;
    const deviationRate = majorDeviations / multipleDeviations.length;
    // Still moving toward destination despite deviations
    const distanceToDestination = haversineDistance(currentLocation.latitude, currentLocation.longitude, destination.latitude, destination.longitude);
