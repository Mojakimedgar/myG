...
import { collection, doc, addDoc, getDocs, getDoc, updateDoc, query, where, onSnapshot, limit, // Added limit for efficiency
...
export const createInvitation = async (invitationData) => {
    try {
        ...
        const userQuery = query(usersRef, where("email", "==", invitationData.toUserEmail), limit(1));
        ...
        const invitation = {
            ...
            status: "pending",
            invitationType: "primary_to_guardian",
            createdAt: now,
        };
...
export const createGuardianInvitation = async (invitationData) => {
    try {
        const usersRef = collection(db, "users");
        ...
        const invitation = {
            ...
            toUserEmail: invitationData.toUserEmail,
            status: "pending",
            invitationType: "guardian_to_primary",
            createdAt: now,
        };
        const docRef = await addDoc(collection(db, "mygInvitations"), invitation);
        // ... activity logging code ...
...
export const getInvitationsForUser = async (userId) => {
    const q = query(collection(db, "mygInvitations"), where("toUserId", "==", userId), where("status", "==", "pending"));
    ...
    return snapshot.docs.map((docSnap) => {
        ...
        return {
            ...
            invitationType: data.invitationType || "primary_to_guardian",
...
export const acceptInvitation = async (invitationId) => {
    ...
    const invitationType = invitationData.invitationType || "primary_to_guardian";
...
export const subscribeInvitations = (userId, onChange) => {
    const q = query(collection(db, "mygInvitations"), where("toUserId", "==", userId), where("status", "==", "pending"));
