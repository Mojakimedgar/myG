...
import { collection, doc, addDoc, getDocs, getDoc, updateDoc, query, where, onSnapshot, orderBy, } from "firebase/firestore";
...
export const createSafetyCheck = async (checkData) => {
    ...
    setTimeout(async () => {
        ...
        if (checkSnap.exists()) {
            const currentCheck = checkSnap.data();
...
export const respondToSafetyCheck = async (checkId, response) => {
    const checkRef = doc(db, "safetyChecks", checkId);
    const status = response === "safe" ? "safe" : "help_needed";
    await updateDoc(checkRef, {
        status,
        response,
...
export const escalateSafetyCheck = async (checkId, userId) => {
    const checkRef = doc(db, "safetyChecks", checkId);
    await updateDoc(checkRef, {
        status: "escalated",
        escalatedAt: new Date(),
...
export const getPendingSafetyChecks = async (userId) => {
    const q = query(collection(db, "safetyChecks"), where("userId", "==", userId), where("status", "==", "pending"), orderBy("createdAt", "desc"));
...
export const subscribePendingSafetyChecks = (userId, onChange) => {
    const q = query(collection(db, "safetyChecks"), where("userId", "==", userId), where("status", "==", "pending"), orderBy("createdAt", "desc"));
