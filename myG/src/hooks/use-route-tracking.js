...
import { calculateRouteDeviation, getDeviationSeverity, isMovingTowardDestination, estimateTimeToDestination, calculateRouteMatchPercentage, createRecordedRoute, haversineDistance, } from "@/lib/route-tracking";
import { saveRecordedRoute, saveRouteDeviation, createRouteAlert, saveActiveRoute, updateActiveRoute, getRecordedRoutes, } from "@/lib/route-firestore";
import { showNotification } from "@/lib/notifications";
import { calculateDistance } from "@/lib/zone-detection";
...
export function useRouteTracking(options) {
    const { kidId, destinationId, destinationName, destinationCoord, trackerId, zone, recordingInterval = DEFAULT_RECORDING_INTERVAL, deviationThreshold = DEFAULT_DEVIATION_THRESHOLD, autoRecord = true, onDeviationDetected, onRouteMatched, onDestinationReached, onZoneExit, } = options;
    ...
    const recordedRoutesRef = useRef([]);
    ...
    const handleLocationUpdate = async (location) => {
        const { destinationCoord: destCoord, destinationId: destId, destinationName: destName, zone: activeZone, } = destinationRef.current;
        const currentPoint = {
        ...
        // ============ ZONE EXIT DETECTION ============
        if (activeZone) {
            const isCurrentlyInZone = calculateDistance(currentPoint.latitude, currentPoint.longitude, activeZone.latitude, activeZone.longitude) <=
                (activeZone.radius || 0);
            // Check if exited zone
            if (state.isInZone && !isCurrentlyInZone && !zoneExitAlertedRef.current) {
                zoneExitAlertedRef.current = true;
                const distanceFromZone = calculateDistance(currentPoint.latitude, currentPoint.longitude, activeZone.latitude, activeZone.longitude);
        ...
        const distToDestination = haversineDistance(currentPoint.latitude, currentPoint.longitude, destCoord.latitude, destCoord.longitude);
        const estDistance = distToDestination;
        const estTimeRemaining = estimateTimeToDestination(currentPoint, destCoord, 10);
        ...
        try {
            ...
            if (recordedRoutes.length > 0) {
                ...
                recordedRoutes.forEach((route) => {
                    const matchPercentage = calculateRouteMatchPercentage(state.currentPoints, route);
                ...
                if (deviation > deviationThreshold && severity !== "minor") {
                    ...
                    try {
                        ...
                        if (lastLocationRef.current &&
                            isMovingTowardDestination(currentPoint, lastLocationRef.current, destCoord)) {
                            // Create alert
                            const alert = {
    ...
    const startTracking = async () => {
        ...
        const { destinationId: destId, destinationName: destName, destinationCoord: destCoord, zone: activeZone } = destinationRef.current;
        // Reset state
        ...
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            watchIdRef.current = navigator.geolocation.watchPosition(handleLocationUpdate, handleLocationError, {
    ...
    const recordRoute = async () => {
        ...
        if (state.currentPoints.length < 10) {
            console.warn("Not enough points to create a recorded route (minimum 10)");
            return;
        }
        const now = new Date();
        const dayOfWeek = now.getDay();
        const route = createRecordedRoute(kidId, destId, destName, state.currentPoints, dayOfWeek);
        try {
